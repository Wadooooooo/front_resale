...
// ... (Компоненты Modal и ControlWithButton остаются без изменений)
...
function CashFlowPage() {
    ...
    // --- ИЗМЕНЕНИЕ 1: Добавляем состояние для видимости истории ---
    const [isHistoryVisible, setIsHistoryVisible] = useState(false);
    const [selectedAccountId, setSelectedAccountId] = useState(null);

    // --- ИЗМЕНЕНИЕ 2: Обновляем обработчик клика ---
    const handleAccountCardClick = (accountId) => {
        // Если кликаем по уже активной карточке, скрываем таблицу
        if (selectedAccountId === accountId) {
            setIsHistoryVisible(false);
            setSelectedAccountId(null);
        } else {
        // Иначе — устанавливаем новый фильтр и показываем таблицу
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return transactions.filter(t => t.account?.id === selectedAccountId);
    }, [transactions, selectedAccountId]);

    // ... (остальные состояния и обработчики остаются без изменений)
    ...
    const loadData = async () => {
        try {
            ...
            const [transData, catData, countData, accWithBalancesData, totalBalanceData, inventoryData] = await Promise.all([
                getCashFlows(), getOperationCategories(), getCounterparties(), 
                getAccountsWithBalances(), getTotalBalance(), getInventoryValuation()
            ]);
    ...
    const handleTransferSubmit = async (e) => {
        ...
        if (!withdrawalCategory || !depositCategory) {
            setError('Необходимые категории для перевода не найдены в базе.');
            setMessage('');
            return;
        }
        ...
        try {
            ...
            setIsHistoryVisible(true);
            setSelectedAccountId(null); // Показываем все счета, чтобы были видны обе операции
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!newTransaction.operation_categories_id || !newTransaction.account_id || !newTransaction.amount) {
        ...
        const dataToSend = {
            ...
            operation_categories_id: parseInt(newTransaction.operation_categories_id),
            account_id: parseInt(newTransaction.account_id),
            counterparty_id: newTransaction.counterparty_id ? parseInt(newTransaction.counterparty_id) : null,
        };
        try {
            const createdTransaction = await createCashFlow(dataToSend);
            setMessage('Операция успешно добавлена!');
            setNewTransaction({
            ...
            // Автоматически показываем историю и фильтруем по нужному счету
            setIsHistoryVisible(true);
            setSelectedAccountId(createdTransaction.account.id);
        ...
        <div>
            ...
            <div className="order-page-container">
                ...
                <div className="balances-grid">
                    <div className={`balance-card ${selectedAccountId === null && isHistoryVisible ? 'active' : ''}`} onClick={() => handleAccountCardClick(null)} style={{ cursor: 'pointer' }}>
                        <h4>Общий баланс</h4>
                        <p className={totalBalance >= 0 ? 'balance-positive' : 'balance-negative'}>{parseFloat(totalBalance).toLocaleString('ru-RU')}</p>
                    </div>
                    ...
                    <div className="balance-card" style={{ backgroundColor: '#eef2f6' }}>
                        ...
                        <p className="balance-positive">{parseFloat(inventoryValue).toLocaleString('ru-RU')}</p>
                    </div>
                </div>
            </div>
            
            {/* --- ИЗМЕНЕНИЕ 3: Оборачиваем всю секцию "История операций" в условие --- */}
            {isHistoryVisible && (
                <div className="order-page-container">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h2>
                            История операций 
                            {selectedAccountId !== null && ` (Счет: ${accounts.find(a => a.id === selectedAccountId)?.name})`}
                        </h2>
                        {selectedAccountId !== null && (
                            <button onClick={() => setSelectedAccountId(null)} className="btn btn-secondary" style={{ marginTop: 0 }}>
                    ...
                    <table className="orders-table">
                        ...
                        <tbody>
                            {filteredTransactions.map(t => {
                                ...
                                return (
                                    <tr key={t.id}>
                                        <td>{t.id}</td><td>{new Date(t.date).toLocaleString('ru-RU')}</td><td>{t.operation_category?.name || '-'}</td>
                                        <td>{t.account?.name || '-'}</td><td>{t.counterparty?.name || '-'}</td>
                ...
                <form onSubmit={handleTransferSubmit}>
                     <div className="details-grid">
                        <div className="form-section">
                             ...
                             <Select options={accountOptions} value={transferForm.from_account_id} onChange={(val) => setTransferForm(prev => ({...prev, from_account_id: val}))} placeholder="Счет списания..." isClearable />
                        </div>
                        <div className="form-section">
                             <label>На счет:</label>
                             <Select options={accountOptions.filter(opt => opt.value !== transferForm.from_account_id?.value)} value={transferForm.to_account_id} onChange={(val) => setTransferForm(prev => ({...prev, to_account_id: val}))} placeholder="Счет зачисления..." isClearable />
                        </div>
                        <div className="form-section">
                             <label>Сумма:</label>
                             <input type="number" step="0.01" value={transferForm.amount} onChange={(e) => setTransferForm(prev => ({...prev, amount: e.target.value}))} className="form-input" placeholder="0.00" />
                        </div>
                     </div>
                     <div className="form-section">
                         <label>Комментарий (необязательно):</label>
                         <textarea value={transferForm.description} onChange={(e) => setTransferForm(prev => ({...prev, description: e.target.value}))} className="form-input" rows="2" />
                     </div>
            ...
            <div className="order-page-container">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="details-grid">
                        ...
                        <div className="form-section"> <label>Счет*</label> <Select name="account_id" options={accountOptions} value={accountOptions.find(opt => opt.value === newTransaction.account_id)} onChange={(val, act) => handleSelectChange(val, act)} placeholder="Выберите..." isClearable required components={{ Control: ControlWithButton }} onButtonClick={() => setIsAccountModalOpen(true)} /> </div>
