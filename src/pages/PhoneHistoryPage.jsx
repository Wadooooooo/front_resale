...
function PhoneHistoryPage() {
    ...
    const handleExchangeSubmit = async (e) => { e.preventDefault(); if (!selectedReplacementId) { alert('Пожалуйста, выберите телефон для обмена.'); return; } if (isSubmitting) return; setIsSubmitting(true); try { await processExchange(history.id, selectedReplacementId); setMessage('Обмен успешно выполнен!'); setIsExchangeModalOpen(false); setSelectedReplacementId(''); await findHistoryBySN(serialNumber); } catch (err) { alert(err.response?.data?.detail || 'Не удалось выполнить обмен.'); } finally { setIsSubmitting(false); } };
                 ...
                 <div className="order-page-container">
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start'}}>
                        <div>
                            ...
                            {/* ИСПОЛЬЗУЕМ НОВУЮ ФУНКЦИЮ ДЛЯ ОТОБРАЖЕНИЯ СТАТУСА */}
